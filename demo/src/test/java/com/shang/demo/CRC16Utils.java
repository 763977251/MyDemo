...
public class CRC16Utils {
    ...
    public static String getCRC16(byte[] data) {
        int crc = (byte)0xff;
        int ucCRCHi = (byte)0xff;
        int ucCRCLo = (byte)0xff;
        int iIndex;
        for (int i = 0; i < data.length; ++i) {
            iIndex = (ucCRCLo ^ data[i]) & (byte)0x00ff;
            ucCRCLo = ucCRCHi ^ crc16_h[iIndex];
            ucCRCHi = crc16_l[iIndex];
        }

        crc = ((ucCRCHi & (byte)0x00ff) << 8) | (ucCRCLo & (byte)0x00ff) & (byte)0xffff;
        //高低位互换，输出符合相关工具对Modbus CRC16的运算
        crc = ( (crc & (byte)0xFF00) >> 8) | ( (crc & (byte)0x00FF ) << 8);
